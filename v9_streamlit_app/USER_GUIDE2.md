...
This guide is written so that a researcher or collaborator who has **never seen this project before** can install it, launch the dashboard, and reproduce the manuscript results without further help. If you only want to get the app running, read [Section 0 (Quick start)](#0-quick-start) and [Section 3 (Launching the dashboard)](#3-launching-the-dashboard); the rest is reference material for interpreting what you see on screen.

> This is the beginner-friendly edition. The original, more terse reference is preserved alongside it as `USER_GUIDE.md`; the technical content of both is identical.
...
2. [Prerequisites, environment, and installation](#2-prerequisites-environment-and-installation)
...
10. [Practical workflows (worked examples)](#10-practical-workflows-worked-examples)
...
> Assumes Python 3.10+ is already installed and you have a copy of the **whole** `CLEAR_ATS` repository (not just the `v9_streamlit_app/` folder — see [Section 4](#4-repository-layout--what-the-app-needs-on-disk) for why).

```bash
# 1. Go to the repository root (the folder that contains v9_streamlit_app/, v4_streamlit_app/, scenarios/, configs/, results/)
cd /path/to/CLEAR_ATS

# 2. (Recommended) create and activate an isolated Python environment
python3 -m venv .venv
source .venv/bin/activate            # Windows: .venv\Scripts\activate

# 3. Install the dashboard's dependencies
...
**Expected output:** a few lines in the terminal ending with a local URL such as `http://localhost:8501`, and that page opening automatically in your default browser. Use the page list in the left sidebar to switch between the four screens (**Overview → One-Time Energy → Utility Phase Energy → Scenario Explorer**). Press `Ctrl-C` in the terminal to stop the server.

If anything fails, jump to [Section 12 (Common errors and troubleshooting)](#12-common-errors-and-troubleshooting).

---

## 1. What CLEAR-ATS is — purpose and scope

CLEAR-ATS is a **scenario-conditioned simulation framework**. Given a set of policy and deployment assumptions, it projects the **energy demand (kWh yr⁻¹)** and **CO₂ emissions (kg CO₂ yr⁻¹)** of road transport from 2024 forward, with particular emphasis on how **Connected Autonomous Vehicles (CAVs)** and **Smart Traffic Infrastructure (STI)** reshape fleet-level energy use. (A short glossary of every acronym used here is in [Section 9.1](#91-vehicle--infrastructure-categories).)

You don't need to know the underlying model code to use the dashboard — the dashboard *is* the front end. Behind the scenes it instantiates a year-by-year simulator (`footprint_model.py` / `core.py`), runs it live whenever you change a control, and overlays a Monte-Carlo uncertainty band. The v9 dashboard is the active public-facing front end and is organised into four screens (selectable from the left sidebar):
...
## 2. Prerequisites, environment, and installation

### 2.1 What you need before you start

| Requirement | Detail |
|---|---|
| **Operating system** | Linux, macOS, or Windows. Examples below use a Unix shell; the only Windows-specific difference is the virtual-environment activation command (noted inline). |
| **Python** | **3.10 or newer** (tested on 3.10 and 3.13). Check with `python3 --version`. |
| **pip** | Comes with Python ≥ 3.4. Check with `python3 -m pip --version`. |
| **A web browser** | Streamlit serves a local web app; any modern browser works. |
| **The full repository** | You need the *entire* `CLEAR_ATS` repository checked out, not just `v9_streamlit_app/`. The v9 app imports tested helpers from the sibling `v4_streamlit_app/` folder and reads configuration and pre-computed results from the repo-root `scenarios/`, `configs/`, and `results/` folders. See [Section 4](#4-repository-layout--what-the-app-needs-on-disk). |
| **Disk space** | ~1–2 GB if the large Monte-Carlo run files (`results/*_mc_runs.csv`) are present. The dashboard itself only needs the much smaller `*_quantiles.csv` bundles, which ship with the repo. |
| **Internet** | Only required for `pip install`. The dashboard runs fully offline once dependencies are installed. |

> **Terminology note.** *"Repository root"* in this guide always means the directory that **directly contains** `v9_streamlit_app/`, `v4_streamlit_app/`, `scenarios/`, `configs/`, and `results/`. Every command below is meant to be run from there. Do not confuse it with the nested `CLEAR_ATS/CLEAR_ATS/` sub-folder, which is an older standalone copy and is **not** what v9 uses.

### 2.2 Step-by-step installation

**Step 1 — Get the repository and open a terminal in its root.**

```bash
cd /path/to/CLEAR_ATS          # the folder containing v9_streamlit_app/
ls                              # you should see: v9_streamlit_app  v4_streamlit_app  scenarios  configs  results  ...
```

**Step 2 — Create an isolated Python environment (strongly recommended).** This keeps the dashboard's package versions from clashing with anything else on your machine.
...
|---|---|---|
| `streamlit` | ≥ 1.32.0 | the web-app framework that renders the dashboard |
| `pandas` | ≥ 2.0.0 | tabular data handling (results CSVs, factor tables) |
| `numpy` | ≥ 1.24.0 | numerics and Monte-Carlo sampling |
| `plotly` | ≥ 5.18.0 | the interactive charts |
| `matplotlib` | ≥ 3.7.0 | a few static figures and the shared Nature-style palette |

**Expected output:** pip prints `Successfully installed streamlit-… pandas-… numpy-… plotly-… matplotlib-…` (plus their transitive dependencies). No errors.

**Step 4 — (Optional) sanity-check the install without launching the UI.** From the repository root:

```bash
python scripts/validate_v9_dashboard_runtime.py
```

This is a fast pre-flight that catches the classes of regression that have crashed the live page in the past (mixed-type tables, deprecated Streamlit arguments, etc.). **Expected output:** a series of `[ OK ]`/`[FAIL]` lines ending in a summary; all checks should pass. It does *not* start a web server.

**Step 5 — Launch.** See [Section 3](#3-launching-the-dashboard).

> **Re-entering the environment later.** Every new terminal session, re-activate the environment first: `cd /path/to/CLEAR_ATS && source .venv/bin/activate`. Then `streamlit run v9_streamlit_app/streamlit_app.py`.
...
1. Streamlit starts a local web server and prints something like:
...
2. Your default browser opens that URL automatically. If it does not, copy the **Local URL** into a browser manually.
3. The **Overview** page loads first. Use the **page list in the left sidebar** to move between the four screens. The **Scenario Explorer** opens with the sidebar expanded (it has many controls); the other pages open with the sidebar collapsed (they have few or no sidebar controls).

**Useful launch options:**

```bash
streamlit run v9_streamlit_app/streamlit_app.py --server.port 8502        # use a different port
streamlit run v9_streamlit_app/streamlit_app.py --server.headless true    # don't auto-open a browser (e.g. on a server)
```

**To stop the app:** press `Ctrl-C` in the terminal where it is running.

> **You must launch from the repository root**, with the path `v9_streamlit_app/streamlit_app.py` as written. The app resolves its sibling and parent folders relative to its own location, so running it from inside `v9_streamlit_app/` (e.g. `streamlit run streamlit_app.py`) also works, but copying `v9_streamlit_app/` somewhere else on its own will **not** — see [Section 4](#4-repository-layout--what-the-app-needs-on-disk).

---

## 4. Repository layout — what the app needs on disk

The v9 dashboard is **not** a self-contained folder. It depends on several siblings and parents inside the `CLEAR_ATS` repository. Here is the minimal picture of what must be present:
...
│   ├── requirements.txt                      ← the dependency list you pip-install
...
│   │   └── 03_Scenario_Explorer.py           ← the main interactive screen
│   ├── utils/                                ← small helpers (factor tables, chart guards)
│   ├── configs/                              ← v9-local config: policy_scenarios.json, mitigation_defaults.json,
│   │   │                                       parameter_labels.json, weather_v8/ (state weather priors)
│   │   └── weather_v8/
│   └── assets/
│       └── clear_ats_uncertainty_figure_v30.png   ← the schematic shown on the Overview page
│
...
└── results/                                  ← pre-computed Monte-Carlo bundles the dashboard displays
    ...
    └── …__mc_runs.csv / …__metrics.csv (larger; optional but used for some panels)
```

**Why this matters for reproducibility:**

- **`v4_streamlit_app/`** must exist alongside `v9_streamlit_app/`. The v9 `core.py` loads `v4_streamlit_app/core.py` at import time for the tested simulation and config-plumbing helpers. If it is missing, the dashboard fails to start.
- **`scenarios/`** (preferred) and **`configs/`** (legacy fallback) hold the regional model parameters (2024 starting values, growth rates, consumption rates, emission factors). The app tries `scenarios/{region}/scenario.json` first, then `configs/{region}.json`.
- **`results/…_quantiles.csv`** are the *committed Monte-Carlo bundles*. They ship with the repo and are what Figure A on the Scenario Explorer shows **before** a live band is computed for your current settings. The `"Default"` bundle is `bundle-default_quantiles.csv`; the `"Alternative reproduction (paper-safe)"` bundle is `bundle-paper-safe_quantiles.csv`. You do **not** need to regenerate these to use the dashboard; see [Section 11](#11-reproducing-the-manuscript-exactly) if you want to.
- The dashboard never *writes* to these files; live bands are recomputed in memory each time you change a setting.

> **Bottom line:** to run v9, keep the whole `CLEAR_ATS` repository intact and launch from its root.

---

## 5. Overview page

A static landing page — there are no interactive controls. It contains:

- A one-paragraph description of the framework.
- The **uncertainty-structure figure** (`v9_streamlit_app/assets/clear_ats_uncertainty_figure_v30.png`) — a schematic that traces uncertainty from input data and state conditions → unit-level demand → fleet-scale propagation → resulting energy and CO₂ ranges. The factor-reference tables on the Scenario Explorer are grouped to mirror this figure. *(If the PNG is missing, the page shows an info box instead of crashing — the figure is expected to be present alongside the dashboard.)*
- A three-column "How to read this dashboard" key that summarises the other three pages.

**Expected output:** the title "CLEAR-ATS", a caption, a paragraph, the schematic image, and a three-column key. Nothing here changes when you click.
...
**Expected output:** Figures A, B, C; four metric cards; the inversion panel with two donuts; the tornado chart; and the three collapsed reference expanders. No values change unless you toggle the Figure C view radio or open an expander — there are no sliders on this page.
...
**Expected output:** the two number inputs and the region selectbox at the top; Figure 1; Figure 2; the "Reading this page" bullets; and the read-only uncertainty tables. Editing the propulsion numbers updates Figure 1 immediately; changing the rate region updates the AV-subsystem segments.

---

## 8. Scenario Explorer page — the main interactive screen

This page is the heart of the dashboard. It runs the simulator live on every interaction, draws the deterministic trajectory immediately, and rebuilds the Monte-Carlo uncertainty band automatically whenever any setting changes. **There is no "Recompute" button — the band updates on its own; a short spinner ("Updating residual band for the selected settings…") appears while it does.**

**Layout, top to bottom:** **sidebar** (scope + scenario settings) → **main panel** (scope notes, fixed-data and structural-assumption expanders, Figure A, Figure B, Figure C, scenario-setting leverage, subsystem decomposition, factor reference, residual-uncertainty-range controls, full provenance).
...
| **Default residual band** | Default, Alternative reproduction ("paper-safe") | Which committed Monte-Carlo bundle backs Figure A *before* a live band has been computed for the current settings. "Default" is the main evidence-anchored bundle (`results/{region}__policy-baseline__bundle-default_quantiles.csv`); "Alternative reproduction" is the paper-safe bundle (`…bundle-paper-safe_quantiles.csv`), retained for comparison. |

### 8.2 Sidebar — State weather profile (expander, collapsed by default)
Implements the annual state weather-share Dirichlet (factors **F32–F36**). A *Dirichlet distribution* here just means: the three weather shares (clear / cloudy / adverse) are drawn at random each year around a target that sums to 1, with a "concentration" parameter κ controlling how tightly they cluster around that target. Two modes (radio):
...
**"Reset to state defaults" button** — returns the six sliders to the current region's defaults and rebuilds the band.

> **Live behaviour:** changing any slider redraws the deterministic trajectory instantly. The default committed residual band does *not* re-centre on the new scenario; instead the page recomputes a fresh residual band (and scenario envelope) keyed on the full settings signature.

### 8.4 Main panel — scope notes and warnings
...
### 8.5 Main panel — "Fixed data — measured 2024 starting values" (expander)
...
### 8.6 Main panel — "Structural assumptions — model-design choices" (expander)
...
### 8.7 Main panel — Figure A: ATS trajectory
...
**Expected output:** on first load you see the deterministic trajectory immediately, then (after a brief spinner) the live residual band fills in and the green "band updated for current settings" line appears. Every metric card and figure on the page is keyed to the *current* sidebar settings; nothing requires a manual recompute.

---

## 9. Key concepts and glossary

If a term on screen is unfamiliar, look it up here first.
...
- **ATS** — the Automated Road Transport System as a whole (ECAV + ICECAV + STI). "ATS energy"/"ATS emissions" are the fleet-level totals plotted in Figure A on the Scenario Explorer.
...
### 9.2 Layers (used in Figures B and C on the Scenario Explorer)
...
- **Default / committed bundle** — a pre-computed Monte-Carlo bundle stored on disk (`results/{region}__policy-baseline__bundle-*_quantiles.csv`) used to populate Figure A before a live band exists. Centred on the *baseline* policy and the region defaults; it is *not* re-centred when you change scenario settings — the page computes a fresh live band instead.
...
- **Monte-Carlo band** — `n` runs (the "Monte Carlo runs" slider) with the residual ranges (and, for the envelope, the targets) sampled; percentiled per year (or per cumulative run for cumulative metrics). *Monte Carlo* simply means "run the model many times with random draws of the uncertain inputs, then summarise the spread of outcomes."
...
## 10. Practical workflows (worked examples)

Each example is a short click-path on the **Scenario Explorer** page (unless stated otherwise).

**"What if California pushes BEV adoption and a clean grid harder?"**
Sidebar: Region = California, Policy = Baseline → either open the *Policy case* expander and apply *CA-Aggressive*, or directly raise *Annual BEV-share growth* (F25) and *Annual low-carbon electricity share growth* (F26) → watch the dark deterministic line on Figure A drop; the residual band rebuilds automatically. Compare the *Peak year* and *Turning year* metric cards before/after.
...
Open *Residual uncertainty ranges* → set the parameter to *Customized*, edit the spec; an invalid spec is flagged and ignored until corrected. The "All default ranges active" badge reads **No** while any custom spec is active.

---

## 11. Reproducing the manuscript exactly

To put the dashboard in a paper-matching configuration:

1. Open the **Scenario Explorer**.
2. **Region** ∈ {California, Ohio} (do **not** use U.S. Average).
3. **Policy** = Baseline.
4. **Default residual band** = Default (or "Alternative reproduction" for the paper-safe bundle).
5. **State weather profile** = Auto (state default).
6. All entries in **Residual uncertainty ranges** = Default.
7. All **Structural assumptions** at their defaults — *Balanced* CAV mix, *Basic-heavy* STI mix, 12-yr service life, *Linear* fleet growth.
8. The **"All default ranges active = Yes"** badge in the Residual-uncertainty-ranges section confirms you are in a paper-matching configuration.

**Regenerating the committed bundles (optional).** The `results/…_quantiles.csv` bundles ship with the repo, so you do **not** need to regenerate them to reproduce the manuscript. If you want to rebuild them from scratch (e.g. after editing a `scenarios/{region}/scenario.json` parameter), run, from the repository root:

```bash
python scripts/regenerate_default_bundle_quantiles.py     # rebuilds CA + OH bundle-default (and paper-safe) quantile CSVs
```

or, for raw deterministic / Monte-Carlo runs of the underlying engine:

```bash
python footprint_model.py --scenarios california ohio --years 68 --policy baseline --mc 200 --seed 42
```

**Expected output:** updated `results/{region}__policy-baseline__bundle-default_quantiles.csv` (and friends). The dashboard picks them up the next time it loads. *Note:* `*_mc_runs.csv` files can be GB-scale; the dashboard itself only needs the small `*_quantiles.csv` files.
...
|---|---|---|
| `streamlit: command not found` | The virtual environment is not active, or dependencies were never installed. | `cd /path/to/CLEAR_ATS && source .venv/bin/activate`, then `pip install -r v9_streamlit_app/requirements.txt`. As a fallback, `python -m streamlit run v9_streamlit_app/streamlit_app.py`. |
| `ModuleNotFoundError: No module named 'streamlit'` (or `pandas`, `plotly`, `numpy`, `matplotlib`) | Dependencies not installed into the active interpreter. | Re-run `pip install -r v9_streamlit_app/requirements.txt` with the right environment active. Confirm with `python -c "import streamlit, pandas, numpy, plotly, matplotlib; print('ok')"`. |
| `FileNotFoundError` / `ModuleNotFoundError` mentioning `v4_streamlit_app`, `_v4_core`, `scenarios/…`, `configs/…`, or `core.py` | You launched from outside the repository, or copied `v9_streamlit_app/` out on its own. The v9 app imports `v4_streamlit_app/core.py` and reads `scenarios/`, `configs/`, `results/` from the repo root. | Keep the whole `CLEAR_ATS` repository intact and launch from its root: `cd /path/to/CLEAR_ATS && streamlit run v9_streamlit_app/streamlit_app.py`. See [Section 4](#4-repository-layout--what-the-app-needs-on-disk). |
| Overview page shows "Uncertainty framework figure not found …" info box instead of the schematic | `v9_streamlit_app/assets/clear_ats_uncertainty_figure_v30.png` is missing. | Restore the file from the repository; the rest of the dashboard is unaffected. |
| Scenario Explorer: a yellow warning "the committed bundle is centred on Baseline …" | You selected a Policy other than Baseline. This is expected, not an error — the committed band is not re-centred under presets, but the page still computes a fresh live band. | Ignore it, or switch Policy back to Baseline. |
| Scenario Explorer: a red error about a "quarantined" region; figures missing or flagged | U.S. Average is selected. It is exploratory only (consumption tables diverge 10–30× from CA/OH). | Use California or Ohio for any quantitative reading. |
| Port already in use (`Address already in use` / app opens but is the wrong project) | Another Streamlit (or service) is using port 8501. | `streamlit run v9_streamlit_app/streamlit_app.py --server.port 8502`. |
| Charts feel slow when you drag the "Monte Carlo runs" slider up | More MC samples = more compute per rebuild. | Keep the slider at 60–100 for interactive exploration; raise to 160–200 only when you need a tighter band for a final read. |
| "All default ranges active" badge says **No** but you didn't change anything | A non-Baseline policy, a non-CA/OH region, a non-default bundle, a Custom weather profile, or a Customized residual range is active. | Use the **"Reset all to default ranges"** button, set Policy = Baseline, Region ∈ {CA, OH}, weather = Auto, bundle = Default. See [Section 11](#11-reproducing-the-manuscript-exactly). |
| Browser tab opened but is blank / "connecting…" forever | The server crashed at startup (check the terminal for a traceback) or the browser cached an old session. | Read the terminal output; fix the underlying error (usually one of the rows above), then reload. A hard refresh (`Ctrl-Shift-R`) clears a stale session. |
...
For anyone who wants to trace a value or modify the model rather than just use the dashboard:

| What | File(s) |
|---|---|
| Dashboard behaviour (entry point + pages) | `v9_streamlit_app/streamlit_app.py`, `v9_streamlit_app/pages/01_One_Time_Energy.py`, `pages/02_Utility_Phase_Energy.py`, `pages/03_Scenario_Explorer.py` |
| Simulation / UI bridge (v9-specific) | `v9_streamlit_app/core.py` (re-exports tested helpers from `v4_streamlit_app/core.py`), `v9_streamlit_app/weather_module.py`, `v9_streamlit_app/scenario_definitions.py`, `v9_streamlit_app/one_time_data.py` |
...
| Project-wide context, version history, and the experiment → dashboard pipeline | `CLAUDE.md`, `docs/VERSION_TIMELINE.md`, `docs/EXPERIMENT_TO_DASHBOARD_PIPELINE.md`, `reports/CLEAR_ATS_v8_BRIEFING.md` |
| Pre-flight install check | `scripts/validate_v9_dashboard_runtime.py` |

---

*Document version: v9 dashboard guide (beginner-friendly edition). Source of truth for behaviour: the files listed in Section 14. Companion document: `USER_GUIDE.md` (original concise reference).*
