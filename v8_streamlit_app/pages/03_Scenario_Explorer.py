"""Scenario Explorer — four-block interactive page.

Sidebar   Region / Policy / Bundle + mitigation levers (Block 1)
Main      Block 2 fixed data · Block 3 assumptions · Block 4 residual uncertainty
Figures   A: ATS emissions trajectory · B: residual drivers · C: layer summary
Appendix  Subsystem decomposition over time · Factor specification panel
"""
...
with st.sidebar:
    ...
    bundle_display = st.selectbox(
        ...
        help="Which committed Monte Carlo bundle to display when the "
             "live residual band has not been requested. Default is the "
    ...
    try:
        ...
        if _scen_ids:
            with st.expander("Policy pathway", expanded=False):
                st.caption(
                    "Pick a discrete policy pathway for the selected region. "
                    "Apply to snap the mitigation sliders below to that "
                    "pathway's documented targets."
                )
                ...
                if st.button("Apply pathway to mitigation sliders",
...
st.caption(
    "The residual band carries short-horizon variability that does not "
    "compound with time. Long-horizon divergence between pathways is "
    "reported by switching between discrete policy pathways below, not "
    "by widening a single band."
)

with st.expander("Scope note — read this first",
                 expanded=False):
    st.markdown(
...
st.caption(
    "The Block 4 selectors above control the live-band recompute. The "
    "committed band shown in Figure A uses one of the pre-computed "
    "bundles and is independent of the current session."
)
    ...
    if band_mode == "Scenario envelope":
        ...
        if env is not None:
            ...
            if stored is None or stored == _current_signature():
                st.success("Live scenario-envelope band matches current settings")
            else:
                st.warning("Scenario-envelope band is stale — settings have changed since the last recompute")
        else:
            st.warning("Click Recompute to build the scenario-envelope band")
    else:
        if _status_code == "live":
            st.success(_status_label)
        elif _status_code == "committed":
            st.info(_status_label)
        else:
            st.warning(_status_label)
with s2:
...
# Compact panel: one expander per block with a slim factor view.
_compact_cols = ["ID", "Short label", "Treatment", "Value or range"]
_blocks_in_order = [
    "Block 1 — Scenario levers",
    "Block 2 — Fixed data",
    "Block 3 — Structural assumptions",
    "Block 4 — Residual L1 / L2 priors",
]
for _block in _blocks_in_order:
    _sub = _factor_df[_factor_df["Block"] == _block]
    if _sub.empty:
        continue
    with st.expander(_block, expanded=False):
        st.dataframe(_sub[_compact_cols].reset_index(drop=True),
                     hide_index=True, use_container_width=True)

with st.expander("Data sources and assumptions (full provenance)", expanded=False):
...
st.markdown("---")
st.caption(
    "Scenario Explorer — mitigation levers set the scenario; fixed data "
    "and assumptions anchor the model; residual uncertainty priors drive "
    "the live Monte Carlo band. Display horizon terminates at 2075."
)
