...
# ── page config ──────────────────────────────────────────────────────
st.set_page_config(
    page_title="Scenario Explorer v6",
    page_icon="C",
    layout="wide",
...
# v6 — single-layer weather module defaults. Off by default so v6 reproduces
# v5 outputs to floating-point tolerance on first load.
st.session_state.setdefault("expv6_weather_enabled", False)
st.session_state.setdefault("expv6_weather_mode", "deterministic")
st.session_state.setdefault("expv6_weather_concentration", 100.0)
st.session_state.setdefault("expv6_weather_seed", 20260422)
st.session_state.setdefault("expv6_weather_apply_co2", True)
...
with st.sidebar:
    ...
    # F29 — Hardware deployment lag. Placed next to F27 because the two
    # are tightly coupled: F27 sets the frontier slope, F29 delays when
    # the fleet realizes it.
    ...
    # ── v6 weather module — single annual distribution layer ──────────
    st.markdown("---")
    st.markdown("### Weather module  *(v6)*")
    st.caption(
        "Single-layer state-conditioned weather-share modifier applied "
        "to utility-phase energy and grid CO₂. Off reproduces the v5 "
        "dashboard numerically. No second uncertainty object is added."
    )
    _weather_enabled_prev = bool(st.session_state.get("expv6_weather_enabled", False))
    _weather_mode_prev = str(st.session_state.get("expv6_weather_mode", "deterministic"))
    _weather_conc_prev = float(st.session_state.get("expv6_weather_concentration", 100.0))
    _weather_seed_prev = int(st.session_state.get("expv6_weather_seed", 20260422))
    _weather_co2_prev = bool(st.session_state.get("expv6_weather_apply_co2", True))

    st.checkbox(
        "Enable weather module",
        key="expv6_weather_enabled",
        help="When off, v6 outputs match v5 exactly. When on, the annual "
             "weather-share vector reweights subsystem energy and the "
             "grid-intensity CO₂ term.",
    )
    if st.session_state.get("expv6_weather_enabled", False):
        st.selectbox(
            "Weather mode",
            ["deterministic", "stochastic"],
            key="expv6_weather_mode",
            format_func=lambda v: {"deterministic": "Deterministic climatology",
                                   "stochastic":    "Stochastic yearly draw"}[v],
            help="Deterministic uses the state monthly-centroid expectation "
                 "(a single reference trajectory). Stochastic draws one "
                 "Dirichlet-realisation weather-share per year.",
        )
        if st.session_state.get("expv6_weather_mode", "deterministic") == "stochastic":
            st.number_input(
                "Random seed",
                min_value=0, max_value=2_000_000_000, step=1,
                key="expv6_weather_seed",
                help="Seed for the Dirichlet draws. Same seed reproduces "
                     "the same weather trajectory.",
            )
            with st.expander("Advanced — Dirichlet concentration", expanded=False):
                st.slider(
                    "Concentration",
                    min_value=5.0, max_value=500.0, step=5.0,
                    key="expv6_weather_concentration",
                    help="Higher values pin draws close to the monthly "
                         "centroid. The default of 100 gives mild year-to-"
                         "year variation. There is still only one weather "
                         "uncertainty layer.",
                )
        st.checkbox(
            "Apply weather to grid CO₂ as well as energy",
            key="expv6_weather_apply_co2",
            help="If off, only subsystem energy is reweighted; the grid "
                 "intensity is held at the baseline climatology.",
        )

    # Invalidate bands if any weather knob changed, so the live band
    # recomputes under the new weather settings.
    _weather_changed = (
        bool(st.session_state.get("expv6_weather_enabled", False)) != _weather_enabled_prev
        or str(st.session_state.get("expv6_weather_mode", "deterministic")) != _weather_mode_prev
        or float(st.session_state.get("expv6_weather_concentration", 100.0)) != _weather_conc_prev
        or int(st.session_state.get("expv6_weather_seed", 20260422)) != _weather_seed_prev
        or bool(st.session_state.get("expv6_weather_apply_co2", True)) != _weather_co2_prev
    )
    if _weather_changed:
        _invalidate_bands()
...
st.caption(
    ...
    "v6 adds a single-layer state-conditioned annual weather-share "
    "modifier on utility energy and grid CO₂ (sidebar toggle); this is a "
    "calculation update, not an additional two-layer uncertainty object."
)
...
with st.expander(":information_source: Scope note — read this first",
    ...
    st.markdown(
        ...
        "levers (F23 to F27, F29) over evidence-based MEDIUM priors. "
...
def _current_signature() -> tuple:
    ...
    weather_sig = settings_signature(get_session_weather_settings())
...
if recompute_clicked:
    ...
    if band_mode == "Scenario envelope":
        ...
        st.session_state["expv5_envelope_band_meta"] = {
            ...
            "region": region,
            "weather": settings_signature(_weather_settings),
        }
    else:
        with st.spinner("Recomputing residual band..."):
            live_band = compute_live_residual_band_v6(
                _live_cfg, region=region, weather=_weather_settings,
                years=68, n_samples=80,
...
if qf is None or qf.empty:
    ...
    if all(c in qf_display.columns for c in [p05c, p50c, p95c]):
        ...
        try:
            _det_df = run_simulation_v6(
                _live_cfg, years=68, region=region,
                weather=get_session_weather_settings(),
            )
...
with st.expander(
    ...
    try:
        _det_df_full = run_simulation_v6(
            _live_cfg, years=68, region=region,
            weather=get_session_weather_settings(),
        ).set_index("Year")
