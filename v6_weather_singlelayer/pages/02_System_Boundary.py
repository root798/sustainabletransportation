"""System Boundary page — v6.

Identical in content to v5 aside from one scope note clarifying that the
v6 Scenario Explorer now includes a state-conditioned annual weather-share
stochastic modifier inside the utility-phase energy and CO2 calculation.
The system boundary itself is unchanged: manufacturing, logistics, and
end-of-life phases remain out of scope.
"""
...
st.set_page_config(page_title="System Boundary v6", page_icon="C", layout="wide")
...
**v6 note.** The Scenario Explorer in v6 includes a state-conditioned
annual weather-share stochastic modifier inside the utility-phase energy
and CO₂ calculation. It is a single annual distribution-based
adjustment (one weather-share vector per year, drawn from each state's
monthly climatology), not an additional two-layer uncertainty object.
Weather does not propagate into production, logistics, or end-of-life
phases.
"""
)
