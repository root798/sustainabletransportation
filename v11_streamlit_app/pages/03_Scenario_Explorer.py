...
# v11.1 — Scenario Explorer band plumbing. The new module is the single
# source of truth for what changes invalidate the Monte Carlo run.
# Importing from a "src/" layout requires adding the repo root to the
# import path.
_REPO_ROOT = APP_DIR.parent
_SRC_PATH = _REPO_ROOT / "src"
if str(_SRC_PATH) not in sys.path:
    sys.path.insert(0, str(_SRC_PATH))
from clearats.bands import (  # noqa: E402
    BandSource, ScenarioSettings, settings_hash,
    compute_residual_band as bands_compute_residual,
    compute_scenario_envelope_band as bands_compute_envelope,
    cumulative_percentiles,
    EMISSIONS_COL as _CR_EMISSIONS_COL,
    ENERGY_COL as _CR_ENERGY_COL,
)
...
# Shared page-top spacing CSS (see utils.plotly_layout.page_top_spacing).
...
# v11.1 — band-source plumbing. ``residual_hash`` is the SHA-1 (12 hex)
# of the active ``ScenarioSettings`` whose Monte Carlo result is
# currently stashed in ``residual_band`` (a ``BandSource``). When the
# live settings hash differs from ``residual_hash`` and either the
# auto-recompute threshold permits or the user clicks "Recompute band",
# the band is rebuilt. The "Reset to state defaults" button clears
# both keys so the next interaction triggers a fresh compute.
st.session_state.setdefault("residual_hash", None)
st.session_state.setdefault("residual_band", None)
st.session_state.setdefault("envelope_hash", None)
st.session_state.setdefault("envelope_band", None)
# Auto-recompute threshold. Below this many MC runs the band updates on
# every slider change without prompting; at or above it, the user must
# click the "Recompute band" button so the page stays responsive.
_AUTO_RECOMPUTE_THRESHOLD = 200
...
def _invalidate_bands() -> None:
    ...
    # v11.1: also drop the new hash-keyed band caches so the next
    # interaction forces a fresh Monte Carlo run. Without this, clicking
    # "Reset to state defaults" would leave a stale BandSource visible
    # until the user touches another slider.
    st.session_state["residual_hash"] = None
    st.session_state["residual_band"] = None
    st.session_state["envelope_hash"] = None
    st.session_state["envelope_band"] = None
...
if region == "us_average":
    st.warning(
        "**U.S. Average is a synthetic midpoint, not official national data.** "
        "It uses the same per-unit utility-phase model as "
        "California and Ohio (per-unit hardware power is region-independent); "
        "only the fleet size, grid emission factor, and weather climatology "
        "differ. Treat it as illustrative."
    )
if policy != "baseline":
    st.warning(
...
with st.expander("Fixed data — measured 2024 starting values",
    ...
    if adv:
        ...
        st.dataframe(prepare_for_streamlit(pd.DataFrame(rows_f)),
...
with mc_col:
    ...
    mc_runs = st.slider(
        "Monte Carlo runs",
        min_value=20, max_value=1000, step=20,
        key="expv5_mc_runs",
        help=("Higher counts narrow Monte Carlo error in the band but "
              "increase compute time. Below "
              f"{_AUTO_RECOMPUTE_THRESHOLD} runs the band updates "
              "automatically on every slider change; above this "
              "threshold, click **Recompute band** to update."),
    )

# v11.1 — manual-recompute escape hatch. Placed under the slider so the
# expensive recompute is one click away when the user has dialled
# Monte-Carlo runs above the auto-recompute threshold. Below the
# threshold the button is disabled (recompute is automatic).
_rb_left, _rb_right = st.columns([0.7, 0.3])
with _rb_right:
    _recompute_clicked = st.button(
        "Recompute band",
        type="primary",
        use_container_width=True,
        key="expv5_recompute_band",
        help=(f"Force a fresh Monte Carlo run. Required when "
              f"Monte Carlo runs ≥ {_AUTO_RECOMPUTE_THRESHOLD}; "
              "below that the band updates automatically on every "
              "slider change."),
    )

# v11.1 — build the ScenarioSettings record (the SINGLE source of truth
# for cache invalidation) from the live page state. ``_current_signature``
# already covers every residual layer; we just package it into
# ScenarioSettings + take its SHA-1 hash. Any change to any slider /
# radio / weather / template / region / policy / bundle / metric /
# uncertainty object / MC-run count produces a different hash.
def _build_settings() -> ScenarioSettings:
    levers_sig, tmpls_sig, radios_sig, custom_sig, weather_sig = (
        _current_signature()[3], _current_signature()[4],
        _current_signature()[5], _current_signature()[6],
        _current_signature()[7],
    )
    return ScenarioSettings(
        state=str(region),
        policy=str(policy),
        uncertainty_object=str(band_mode),
        metric=str(_metric_id),
        n_monte_carlo=int(mc_runs),
        random_seed=20260518,
        cav_target_2075=round(float(
            st.session_state.get("expv5_cv_cav_growth_rate", 0.0) or 0.0), 8),
        sti_coverage_2075=round(float(
            st.session_state.get("expv5_cv_sti_growth_rate", 0.0) or 0.0), 8),
        bev_share_growth=round(float(
            st.session_state.get("expv5_cv_ev_growth_rate", 0.0) or 0.0), 8),
        low_carbon_elec_growth=round(float(
            st.session_state.get("expv5_cv_clean_energy_growth_rate", 0.0) or 0.0), 8),
        hw_doubling_years=round(float(
            st.session_state.get("expv5_cv_efficiency_doubling_years", 0.0) or 0.0), 8),
        hw_deployment_lag_years=round(float(
            st.session_state.get("expv5_cv_hardware_deployment_lag_years", 0.0) or 0.0), 8),
        levers_blob=json.dumps(list(levers_sig), default=str),
        radios_blob=json.dumps(list(radios_sig), default=str),
        templates_blob=json.dumps(list(tmpls_sig), default=str),
        weather_blob=json.dumps(list(weather_sig), default=str),
        custom_blob=json.dumps(list(custom_sig), default=str),
        bundle_display=str(bundle_display),
    )


_settings = _build_settings()
_current_hash = settings_hash(_settings)

# v11.1 — hash-keyed cache wrappers. The hash itself is the only arg
# that participates in the Streamlit cache key (the underscore-prefixed
# args are intentionally skipped, but unlike the legacy code, the hash
...
# NOTE: Streamlit @st.cache_data ignores arguments whose name starts
# with "_". We rely on that for live_cfg, settings_dict, region, n —
...
# Decide whether to recompute. The user clicked Recompute, OR the
# settings hash has drifted from the cached one AND we're below the
# auto-recompute threshold.
_active_hash_key = (
    "envelope_hash" if band_mode == "Scenario envelope" else "residual_hash"
)
_active_band_key = (
    "envelope_band" if band_mode == "Scenario envelope" else "residual_band"
)
_last_hash = st.session_state.get(_active_hash_key)
_needs_recompute = (_current_hash != _last_hash)
_can_auto = (int(mc_runs) <= _AUTO_RECOMPUTE_THRESHOLD)

_should_run = _needs_recompute and (_can_auto or _recompute_clicked)
...
if _should_run:
    ...
    with st.spinner(_spinner_msg):
        try:
            if band_mode == "Scenario envelope":
                _band_source = _cached_envelope_band(
                    _cache_key_arg(_current_hash), asdict(_settings),
                    _live_cfg, region, int(mc_runs),
                )
            else:
                _band_source = _cached_residual_band(
                    _cache_key_arg(_current_hash), asdict(_settings),
                    _live_cfg, region, int(mc_runs),
                )
            ...
            _band_source = None
            st.warning(
                f"Monte Carlo computation failed: {_exc}. The committed "
                "band file is shown as a fallback."
            )
    st.session_state[_active_hash_key] = _current_hash
    ...
    st.warning(
        f"Settings changed. Click **Recompute band** to update the "
        f"{int(mc_runs)}-run {band_mode.lower()} band. "
        f"(Auto-recompute is enabled for Monte Carlo runs ≤ "
        f"{_AUTO_RECOMPUTE_THRESHOLD}.)"
    )

# Always read the band from session state — it stays valid until the
# settings hash drifts (above), at which point the warning shows.
_band_source: Optional[BandSource] = st.session_state.get(_active_band_key)

# Backward compatibility shim: legacy downstream code expects an
# ``_active_band`` DataFrame with ``{metric}_p05/p50/p95`` columns.
def _band_source_to_legacy_df(bs: Optional[BandSource]):
    if bs is None or not bs.by_metric:
        return None
    import pandas as _pd
    first_ts = next(iter(bs.by_metric.values()))
    out_cols: dict[str, "np.ndarray"] = {}
    for col, ts in bs.by_metric.items():
        out_cols[f"{col}_p05"] = ts.p05
        out_cols[f"{col}_p50"] = ts.p50
        out_cols[f"{col}_p95"] = ts.p95
    return _pd.DataFrame(out_cols, index=_pd.Index(first_ts.years, name="Year"))


_active_band = _band_source_to_legacy_df(_band_source)
...
# v11.1 — cumulative views re-percentile the LIVE Monte Carlo
# trajectories. Earlier versions read a static cumulative CSV
# (``cumulative_band_from_mc_runs``), which never updated when the
# scenario sliders changed and therefore frozen the cumulative band.
# By integrating per-run trajectories before percentile-ing across runs,
# the cumulative band widens monotonically with year and reflects every
# slider change. The committed-cumulative path remains as a fallback
# when no live BandSource is available (e.g. very first page load
# before the user touches anything).
def _live_cumulative_qf(band_source: Optional[BandSource],
                         metric_col: str):
    if band_source is None:
        return None
    ts = band_source.for_metric(metric_col)
    if ts is None or ts.trajectories.size == 0:
        return None
    qs = cumulative_percentiles(ts.trajectories)
    return pd.DataFrame({
        f"{metric_col}_p05": qs["p05"],
        f"{metric_col}_p50": qs["p50"],
        f"{metric_col}_p95": qs["p95"],
    }, index=pd.Index(ts.years, name="Year"))


if _is_cumulative:
    _live_cum_qf = _live_cumulative_qf(_band_source, metric)
    if _live_cum_qf is not None:
        qf_committed = _live_cum_qf
    else:
        # Fall back to the committed CSV when no live BandSource exists.
        @st.cache_data(show_spinner=False)
        def _cached_cum_band_fallback(reg: str, pol: str, bnd: str, met: str):
            return cumulative_band_from_mc_runs(reg, pol, bnd, met)

        cum_band = _cached_cum_band_fallback(region, policy, bundle_display, metric)
...
# v11.1 — every metric path reads from the same live ``_active_band``
# (or its cumulative re-percentile above). Cumulative branches no
# longer bypass the live MC: the only difference between Annual and
# Cumulative is whether ``_live_cumulative_qf`` re-percentiles the
# trajectories before plotting.
...
if qf is None or qf.empty:
    ...
    if all(c in qf_display.columns for c in [p05c, p50c, p95c]):
        ...
        # is not the primary. v11.1 — for both uncertainty objects we
        # now emit a full p05/p50/p95 trio (not just the median) so the
        # right axis shows a band identical in semantics to the left.
        # The previous behaviour silently dropped to a single axis under
        # the Scenario envelope uncertainty object; the dispatch below
        # makes that impossible because the same code path runs for
        # both uncertainty objects.
        _sec_p50 = None
        _sec_unit = None
        _sec_label = None
        _sec_p05 = None
        ...
        if _metric_id == "both":
            ...
            _sec_p05c = f"{_sec_metric}_p05"
            _sec_p95c = f"{_sec_metric}_p95"
            if _sec_p50c in qf_display.columns:
                ...
                if _sec_p05c in qf_display.columns and _sec_p95c in qf_display.columns:
                    _sec_p05 = qf_display[_sec_p05c] / _sec_fac
                    _sec_p95 = qf_display[_sec_p95c] / _sec_fac
                # Deterministic line for the second metric on y2.
                try:
                    _det_sec_df = run_simulation(_live_cfg, years=68)
                    _det_sec_series = _det_sec_df.set_index("Year")[_sec_metric]
                    if _is_cumulative:
                        _det_sec_series = _det_sec_series.cumsum()
                    _det_sec_em = _det_sec_series / _sec_fac
                    _det_sec_em = _det_sec_em.loc[
                        _det_sec_em.index <= DISPLAY_MAX_YEAR]
                    _sec_det = _det_sec_em
                except Exception:
                    _sec_det = None
        ...
        # Secondary (right-axis) traces for the Both view. v11.1 — emit
        # a full p05/p50/p95 band + median + deterministic line on y2,
        # mirroring the left axis. The previous behaviour drew only a
        # dashdot median, so the dual-axis view silently degraded to one
        # axis when the secondary metric had no band; that contradicted
        # the page's promise that both axes share the same uncertainty
        # object.
        if _sec_p50 is not None and _sec_label is not None:
            ...
            # Optional secondary band (only when p05/p95 are available).
            if _sec_p05 is not None and _sec_p95 is not None:
                fig.add_trace(go.Scatter(
                    x=qf_display.index, y=_sec_p95, mode="lines",
                    line=dict(color=sec_color, width=0.4),
                    name=f"{band_mode} p95 · {_sec_label.split()[-1]}",
                    yaxis="y2", showlegend=False, hoverinfo="skip",
                ))
                fig.add_trace(go.Scatter(
                    x=qf_display.index, y=_sec_p05, mode="lines",
                    line=dict(color=sec_color, width=0.4),
                    fill="tonexty",
                    fillcolor=rgba(sec_color, 0.14),
            ...
            # Secondary deterministic line.
            if _sec_det is not None:
                fig.add_trace(go.Scatter(
...
with st.expander(
    ...
    try:
        ...
        st.dataframe(prepare_for_streamlit(pd.DataFrame(rows)),
                     hide_index=True, width="stretch")
        st.caption(
    ...
    with st.expander(_group_label, expanded=False):
        st.dataframe(
            prepare_for_streamlit(
                _sub[_compact_cols].reset_index(drop=True)
            ),
            hide_index=True, width="stretch",
        )
...
with st.expander("Data sources and assumptions (full provenance)",
                 expanded=False):
    st.dataframe(prepare_for_streamlit(_factor_df),
                 hide_index=True, width="stretch")
    st.caption(
