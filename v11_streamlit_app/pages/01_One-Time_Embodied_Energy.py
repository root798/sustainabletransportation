...
from utils.df_display import (  # noqa: E402
    prepare_for_streamlit, style_with_nan_dash,
)
...
# Injects a small CSS block adding breathing room between H1, figure
...
# Layout: apply the shared CLEAR-ATS hbar layout so the legend sits
# below the x-axis title (not on top of it), outside-bar labels
# (e.g. 654.3) are not clipped, and y-axis labels have automargin.
...
apply_clearats_layout(
    ...
    max_x=max(_max_val, 750.0 / 1.10),  # keep the legacy 750-kWh floor
)
# Preserve our explicit y-axis ordering (not provided by the helper).
fig_a.update_yaxes(
    categoryorder="array", categoryarray=y_labels, showgrid=False,
)
...
for r in unit_rows:
    ...
    _compare_rows.append({
        "Unit":              r["unit"],
        "Live (kWh)":        float(live_v),
        # Pass the raw float (NaN if the manuscript row is missing) so the
        # column has a single numeric dtype. The em-dash is restored at
        # render time by the dash-aware Styler below.
...
with st.expander("Per-level component inventory (Extended Data Tables 3 + 4)",
                 expanded=False):
    st.dataframe(prepare_for_streamlit(inv_df),
                 hide_index=True, width="stretch")
...
with st.expander("Component-level energy inventory (Figure 3a)",
    ...
    st.dataframe(
        prepare_for_streamlit(pd.DataFrame(build_component_rows())),
        hide_index=True, width="stretch",
    )

    st.markdown("**Component counts per unit type**")
    st.caption("Extended Data Tables 3 (CAV) and 4 (STI) combined.")
    # The wide count table mixes integer counts with the em-dash '—' for
    # platform-irrelevant cells (e.g. CAV columns on STI components).
    # prepare_for_streamlit coerces those columns to a single dtype that
    # Arrow accepts, while keeping the on-screen em-dash via the Styler.
    _count_df_raw = pd.DataFrame(build_count_wide_table())
    _count_numeric_cols = [c for c in _count_df_raw.columns
                           if c.startswith(("CAV ", "STI "))]
    _count_df_display = prepare_for_streamlit(
        _count_df_raw, numeric_cols=_count_numeric_cols,
    )
    st.dataframe(
        style_with_nan_dash(
            _count_df_display,
            numeric_cols=_count_numeric_cols,
            fmt="{:.0f}",
        ),
        hide_index=True, width="stretch",
    )
...
with st.expander("One-time accounting assumptions",
    ...
    st.dataframe(prepare_for_streamlit(_assumption_rows),
                 hide_index=True, width="stretch")
...
for _stage in _OT_GROUP_ORDER:
    ...
    st.dataframe(
        prepare_for_streamlit(pd.DataFrame(_stage_rows)[[
            "Factor ID",
            "Factor name",
            "Distribution / range",
            "Affected quantity",
...
with st.expander("Data consistency note", expanded=False):
    ...
    st.dataframe(prepare_for_streamlit(_cc_df),
                 hide_index=True, width="stretch")
    st.caption(
