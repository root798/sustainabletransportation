...
st.info(
    "**v10 recalibration.** Per-unit utility-phase energy is now assembled "
    "bottom-up from the component registry (deployed automotive-silicon "
    "power × component counts × duty × utilization), not the flat config "
    "aggregates used by v3-v9. Absolute ATS energy and emissions trajectories "
    "are therefore ~15-20× lower than in v9; the turning-point years, the "
    "relative sensitivities to hardware efficiency / electrification / "
    "renewable growth, and the California-vs-Ohio contrast are unchanged. "
...
if region == "us_average":
    st.warning(
        ...
        "In v10 it uses the same bottom-up per-unit component registry as "
