...
from _v4_core import (  # type: ignore[import-not-found]  # noqa: F401,E402
    ...
    region_paper_safety,
    rgba as core_rgba,
    run_simulation as _v4_run_simulation_base,  # superseded below for v10
    runtime_diagnostics,
    scale,
...
# ---------------------------------------------------------------------
# v10 — component-registry energy model
# ---------------------------------------------------------------------
# v10 routes the utility-phase energy calculation through the bottom-up
...
# the v10 app dir. See v10_streamlit_app/component_registry.py and
...
def _v10_run_simulation(cfg: dict[str, Any], years: int = DEFAULT_HORIZON) -> pd.DataFrame:
    """v10 deterministic run: TransportModel with the component-registry energy
...
# it is not affected.
_v4_run_simulation = _v10_run_simulation
...
def compute_live_residual_band(
    ...
    for _ in range(int(n_samples)):
        ...
        # v10: perturb the *physical* component parameters (per-component power,
        ...
        _pov = _sample_power_overrides(rng)
    ...
    return pd.DataFrame(cols, index=pd.Index(years_axis, name="Year"))
