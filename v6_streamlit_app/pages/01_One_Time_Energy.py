...
# ── page config ──────────────────────────────────────────────────────
st.set_page_config(
    page_title="One-Time Energy — v5.1",
    page_icon="C",
    layout="wide",
    initial_sidebar_state="collapsed",
)

# v6 sidebar legend (always visible)
try:
...
st.caption(
    ...
    "CLEAR-ATS stands for Carbon Life-cycle Emissions Analysis and "
    "Reduction for Autonomous Traffic Systems. Autonomous Traffic "
    "Systems (ATS) comprise Connected Autonomous Vehicles (CAVs) and "
    "Smart Transportation Infrastructure (STI). Component inventory "
    "from Figure 3a of the manuscript. Unit composition from Extended "
    "Data Tables 3 and 4. Unit totals from Figure 3b. Phase totals "
    "from Table 2."
)

st.info(
    "This page visualises the **production and logistics phase only**. "
    "For utility-phase operational energy projections with "
    "long-horizon scenario envelopes, see the **Scenario Explorer** "
    "page. Together the two pages cover the full life-cycle scope of "
    "CLEAR-ATS."
)
...
st.caption(
    f"Cross-check vs manuscript Figure 3b / Table 2. Dashboard "
    f"reproduces all component-level values from Figure 3a and "
    f"Extended Data Tables 3 and 4. Manuscript aggregated totals "
    f"match the dashboard for "
    f"**{_cc_match_count} of {len(_cc_rows)}** rows. The remaining "
    f"rows reflect manuscript-text aggregation decisions being "
    f"reconciled by the authors; see "
    f"`reports/pre_submission/MANUSCRIPT_ONLY_RECONCILIATIONS.md` "
    f"for resolution status."
)
...
with st.expander("Cross-check details", expanded=False):
    ...
    st.caption(
        "Live values computed from the baseline component table "
        "(Figure 3a) and Extended Data Tables 3 / 4, without any "
        "Block 1 slider adjustment. Any 'manuscript reconciliation' "
        "row is a text-side aggregation question for the authors, "
        "not a dashboard error. See "
...
with st.expander("Block 3. Production and end-of-life assumptions",
    ...
    st.info(
        ":information_source: **Scope caveat.** Four of the five "
        "selectboxes below (manufacturing region, logistics model, "
        "failure fraction φ, computing obsolescence window) are "
        "**documentary only** in the current v5.1 build — they name "
        "the scenario but do not yet feed into the downstream "
        "calculation. Only the refurbishment-energy-ratio α is wired "
        "into the live numbers (through the Block 1 sensing-refurbishment "
        "slider). This is flagged as Major-6 in "
        "`reports/summaries/MASTER_ACADEMIC_VALIDATION_REPORT.md`."
    )
    b3_c1, b3_c2 = st.columns(2)
    with b3_c1:
        st.selectbox(
            "Manufacturing region *(documentary only)*",
            ["Asia-Pacific (default)", "North America", "Europe",
             "Mixed global supply"],
            key="ot_mfr_region",
            help="Selection is currently decorative. When wired, this "
                 "will pick the ecoinvent region applied to mass-based "
                 "scaling per Equation 1.",
        )
        st.selectbox(
            "Logistics model *(documentary only)*",
            ["Sea + truck (default)", "Sea + rail + truck",
             "Air + truck (accelerated)"],
            key="ot_logistics_model",
            help="Selection is currently decorative. The Table 2 "
                 "logistics values are stored as constants; the "
                 "mode-split integral in Equation 3 is not yet "
                 "resolved inside the dashboard.",
        )
        st.selectbox(
            "Refurbishment energy ratio α *(wired)*",
            ["25 % (default, per §4.1.4)", "15 % (aggressive remanufacturing)",
             "40 % (conservative)"],
            key="ot_alpha_choice",
            help="Energy ratio used when sensing and computing "
                 "components are refurbished rather than newly "
                 "manufactured. This choice IS wired into the live "
                 "calculation through the sensing-refurbishment-rate "
                 "slider in Block 1.",
        )
    with b3_c2:
        st.selectbox(
            "Failure fraction φ at end of life *(documentary only)*",
            ["10 % (default)", "5 % (high-reliability)",
             "20 % (accelerated degradation)"],
            key="ot_phi_choice",
            help="Selection is currently decorative. When wired, φ "
                 "will multiply the refurbishment-eligible pool per "
                 "Equation 13.",
        )
        st.selectbox(
            "Computing obsolescence window *(documentary only)*",
            ["8 to 24 months (default, per NVIDIA manufacturing)",
             "36 months (extended hardware refresh)",
             "60 months (modular upgrade path)"],
            key="ot_obsolescence_window",
            help="Selection is currently decorative. When wired, "
                 "shorter windows will cap the achievable computing "
                 "refurbishment-adoption slider in Block 1.",
        )
...
st.caption(
    ...
    "All published priors share the same evidence chain (OpenLCA + "
    "ecoinvent 3.9). The current v5.1 build does not run a live "
    "Monte Carlo over these L1 priors on this page; the selectboxes "
    "document the residual-uncertainty footprint that the committed "
    "Figure B values absorb."
)
...
if _b_any_manuscript_gap:
    st.caption(
        "STI Basic row shows a manuscript-text aggregation question "
        "that the authors are reconciling. Extended Data Table 4 "
        "counts \u00d7 Figure 3a energies sum to 2,747.36 kWh; "
        "Table 2 and Figure 3b list 2,139.77 kWh. See "
        "`reports/pre_submission/MANUSCRIPT_ONLY_RECONCILIATIONS.md`."
    )
...
Block 3 φ selectbox is not currently wired into this calculation
(documented as Major-6 in
`reports/summaries/MASTER_ACADEMIC_VALIDATION_REPORT.md`).
"""
    )
...
st.markdown("---")
st.caption(
    "One-Time Energy and Marginal Components — v5.1.5. Companion page "
    "to the Scenario Explorer. Production + logistics phase only. "
    "Manuscript traceability surfaced in the rebuttal cross-check at "
    "the top of the page. See "
    "audits/final_consistency/ONE_TIME_PAGE_COMPREHENSIVE_FIX_VALIDATION.md "
    "and reports/summaries/ONE_TIME_PAGE_COMPREHENSIVE_FIX_STATUS.md."
)
